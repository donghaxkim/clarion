...
WITNESS_REPORT_1="${WITNESS_REPORT_1:-${REPO_ROOT}/mock-evidence/witness-report-01-pamela-ortiz.pdf}"
WITNESS_REPORT_2="${WITNESS_REPORT_2:-${REPO_ROOT}/mock-evidence/witness-report-02-marcus-reed.pdf}"

PYTHON_BIN="$(find_python)"
OUTPUT_DIR="${OUTPUT_DIR:-$(mktemp -d)}"
RAW_JSON="${OUTPUT_DIR}/inspection.json"
INSPECT_ARGS=(
  --pdf "${WITNESS_REPORT_1}"
  --pdf "${WITNESS_REPORT_2}"
  --case-title "${CASE_TITLE:-Mock witness report inspection}"
  --case-summary "${CASE_SUMMARY:-Two witness statements describing an intersection collision.}"
